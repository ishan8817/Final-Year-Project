...
X = dataset[['Cumulative Instructions', 'IPC_03']]
y = dataset['IPC_19']

# Defining a range of test set sizes
test_set_sizes = [0.2, 0.3, 0.4, 0.5]

# Initialize lists to store test set sizes and corresponding mean squared errors
test_sizes = []
mean_squared_errors = []
...
for test_size in test_set_sizes:
    ...
    predicted_IPC_19 = model.predict(X_test_ts)
    ...
    predicted_IPC_19_sorted = predicted_IPC_19[sorted_indices]
    y_test_sorted = y_test.values[sorted_indices]

    plt.plot(X_test_sorted['Cumulative Instructions'], y_test_sorted, label='Actual IPC E(Small) Core', color='green')
    plt.plot(X_test_sorted['Cumulative Instructions'], predicted_IPC_19_sorted, label='Predicted IPC E(Small) Core', color='red')
    ...
    predicted_Speed_up = X_test['IPC_03'].values[:, np.newaxis] / predicted_IPC_19
    ...
    plt.xlabel('Cumulative Instructions ($\\times10^{11}$)', fontsize=14, labelpad=20)
    plt.ylabel('Speed_up', fontsize=14, labelpad=20)
    plt.title(f'Actual vs Predicted Speed_up')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.gca().xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f"{x / 1e11:.0f}"))
